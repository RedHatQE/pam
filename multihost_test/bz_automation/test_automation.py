...
class TestPamBz(object):
    ...
    @pytest.mark.tier1
    def test_faillock_local_sudo_lockout(multihost, bkp_pam_config, create_user_with_sudo_access):
        """
        :title: Faillock feature configuration and behavior for local sudo user
        :id: 32b4ce84-092f-11f0-9ff4-1ee79fc47b20
        :bugzilla: https://issues.redhat.com/browse/SSSD-7549
        :setup:
        ...
        assert username in initial_output, f"User {username} not found in initial faillock output"
        ...
        failed_attempts = [line for line in faillock_output.splitlines()
                           if username in line and re.search(r'\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}.*TTY', line)]
        assert len(failed_attempts) == 1, f"Expected 1 failed attempt, found {len(failed_attempts)}"
